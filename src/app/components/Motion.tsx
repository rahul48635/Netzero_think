...
export default function Motion(): React.ReactNode {
  ...
  const backgrounds = ["#ced4da", "#d2d4c8", "#f3d5b5"];
  ...
  return (
    <motion.div
      ...
      ref={ProgeressRef}
      className="w-full md:w-[calc(100%_-_15rem)] h-full flex justify-center rounded-4xl self-center justify-self-center shadow-[0px_0px_40px_10px_#00b4d8,0px_0px_10px_5px_#0096c7] px-4"
    >
      <div className="mx-auto flex w-full flex-col gap-40 py-20">
...
const Card = ({ feature }: { feature: feature }) => {
  ...
  const blur = useTransform(scrollYProgress, [0, 0.1, 0.5, 1], [10, 0, 0, 10]);
  ...
  return (
    <motion.div
      ref={ref}
      key={feature.title}
      className="grid grid-cols-1 md:grid-cols-2 items-center gap-10 md:gap-20 py-20 px-4 sm:px-6 md:px-10"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      style={{
    ...
    >
      <motion.div className="flex flex-col gap-5 text-5xl">
        {feature.icon}
        <h2 className="text-4xl font-bold text-gray-500 dark:text-white">
          {feature.title}
        </h2>
      ...
      <motion.div
        style={{ y: offset }}
        className="border-2 border-black flex items-center justify-center p-3 rounded-2xl bg-gradient-to-r from-slate-500 via-cyan-500 to-gray-500 mt-24"
      >
        {feature.content}
      </motion.div>
    </motion.div>
...
const feature: feature[] = [
  ...
  },
  {
    ...
    title: "MSME",
    discription:
      "According to the WTO's research, MSMEs represent 95 per cent of companies across the globe and account for 60 per cent of the world's total employment. They contribute to around 35 per cent of GDP in developing countries and around 50 per cent in developed countries.Around 63 millions of Indian MSME contributes about 29% towards the GDP through its national and international trade of services & goods. Carbon footprinting & ESG practicing would help MSMEs to be market relevant in international competition.",
    content: (
      <Image
