...
export default function JobsAndServicesPage() {
  const jobListings = [
    {
      ...
      description:
        "Assist in brand visibility through social media, content creation, and growth campaigns.",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    window.open(
      `mailto:support@netzerothink.com?subject=New Service Request from ${formData.name}&body=Name: ${formData.name}%0APhone: ${formData.phone}%0AEmail: ${formData.email}%0AQualifications: ${formData.qualification}%0ASkills: ${formData.skills}%0AAttach_CV: [Attach Your CV]`,
    );
  };
      ...
      <section className="w-full h-full relative my-30  p-5 rounded-2xl ">
        <div className="flex w-full h-full gap-5 flex-wrap items-center justify-center">
          {jobListings.map((item, idx) => (
            <Card
              key={idx}
              className="max-w-130 h-110 sm:h-70  bg-gradient-to-r  from-slate-300 to-gray-400 p-5 "
            >
              ...
              <div className="flex flex-col gap-2 h-full w-full">
                <h1 className="font-semibold">• Skills Required</h1>
                <div className="relative left-3 w-full h-full flex flex-wrap gap-2">
                  {item.skills.map((skill, i) => (
                    <span
                      key={i}
                      className="px-2 py-1  text-sm rounded-md shadow text-black text-center justify-self-center self-center bg-gray-200"
                    >
                      {skill}
                    </span>
                  ))}
                </div>
                <h1 className="font-semibold ">• Qualification Required</h1>
